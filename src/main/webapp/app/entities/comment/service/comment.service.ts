...
type RestOf<T extends IComment | NewComment> = Omit<T, 'createTime'> & {
...
export class CommentService {
  ...
  protected convertDateFromClient<T extends IComment | NewComment | PartialUpdateComment>(comment: T): RestOf<T> {
    return {
      ...comment,
      createTime: comment.createTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restComment: RestComment): IComment {
    return {
      ...restComment,
      createTime: restComment.createTime ? dayjs(restComment.createTime) : undefined,
    };
  }

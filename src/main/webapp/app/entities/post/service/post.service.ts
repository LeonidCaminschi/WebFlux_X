...
type RestOf<T extends IPost | NewPost> = Omit<T, 'createTime' | 'updateTime'> & {
...
export class PostService {
  ...
  protected convertDateFromClient<T extends IPost | NewPost | PartialUpdatePost>(post: T): RestOf<T> {
    return {
      ...post,
      createTime: post.createTime?.toJSON() ?? null,
      updateTime: post.updateTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPost: RestPost): IPost {
    return {
      ...restPost,
      createTime: restPost.createTime ? dayjs(restPost.createTime) : undefined,
      updateTime: restPost.updateTime ? dayjs(restPost.updateTime) : undefined,
    };
  }
